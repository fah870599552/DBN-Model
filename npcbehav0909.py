...
class threatCompute():
    ...
    def cluster_and_assign(self ,scene ,df, feature_column, target_column, num_clusters,weights):
        ...
        scene_dict = {
            'brake': {1: 0, 2: 1, 0: 2},
            'cutin': {0: 0, 2: 0, 4: 1, 1: 2, 3: 3},
            'ghost': {0: 0, 1: 1},
            'zuozhi': {2: 0, 0: 1, 3: 2, 1:3}
        }
        ...
        return df
    def threat_kmeans(self, df1:pd.DataFrame,scene):
        if scene == 'brake':
            clusternum = 3
            weights = [1, 1, 1]
            cluster_fe = ['fixed_npc_v_mean','fixed_npc_a_mean','fixed_npc_jerk_std']
            df1 = self.cluster_and_assign(scene, df1, cluster_fe, 'n_thr',clusternum, weights)
        ...
        if scene == 'cutin':
            clusternum = 5
            weights = [2, 1.5, 1]
            cluster_fe = ['npc_heading','fixed_npc_jerk_std','fixed_npc_a_mean' ]
            df1 = self.cluster_and_assign(scene, df1, cluster_fe, 'n_thr', clusternum,weights)
        return df1